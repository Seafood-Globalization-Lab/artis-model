...
raw_prod_old_sp <- artis::rebuild_fao_2023_dat(datadir = datadir_old, filename = raw_prod_old_fn) %>%
...
raw_prod_new_sp <- artis::rebuild_fao_2023_dat(datadir = datadir_new, filename = raw_prod_new_fn) %>%
  select(sciname = species_scientific_name, common_name = species_name_en) %>%
  mutate(sciname = tolower(sciname),
        common_name = tolower(common_name)) %>%
  distinct()
...
raw_prod_new_sp_only %>%
  kable(
    caption = "Species present in new production data only",
    booktabs = TRUE,
    col.names = c("Scientific Name", "Common Name")
  ) %>%
