...
cli::cli_h1("🏃🏾‍➡️ Running {.file 02-artis-pipeline.R}")

# if (run_env == "aws") {
#   artis::get_country_solutions(
#     datadir = datadir,
...
cli::cli_h1("Starting HS{hs_version_run} {.fn get_no_solve_countries} at {Sys.time()}")
...
if (nrow(no_solve_countries) > 0) {
  ...
  # Messages are within get_country_solutions() funciton definition
...
cli::cli_h1("Starting HS{hs_version_run} {.fn get_snet} at {Sys.time()}")
...
cli::cli_h1("🏁 Finished HS{hs_version_run} {.fn get_snet} at {Sys.time()}")