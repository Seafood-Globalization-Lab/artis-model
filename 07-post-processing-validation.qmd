---
title: "ARTIS v1.2.0 SAU Data Validation"
author: "Jessica Gephart"
date: "Compiled on `r format(Sys.time(), '%d %B, %Y')`"
format:
  html:
    embed-resources: true
    output-file: 07-ARTIS-validation_1.2.0_SAU_v7_replace_prod_sau_att.html
---
...
db_folder <- glue("/Users/theamarks/Documents/UW-SAFS/ARTIS/data/outputs_{artis_version}_{prod_type}_{outputdir_date}")

# List all matching files in the directory first (do not need specific file date)
parquet_files <- list.files(
    file.path(db_folder, "outputs_combined"), 
    pattern = glue::glue("ARTIS_v{artis_version}_trade_{prod_type}_mid_all_HS_yrs_*"),
    full.names = TRUE)

# load ARTIS data set
# point to the dataset folder
#artis_ds <- arrow::open_dataset(file.path(db_folder, "datasets", "ARTIS_trade_v1.2.0_SAU_mid_2025-11-12.parquet"))
artis_ds <- arrow::open_dataset(parquet_files[1])
...
artis_ts_result <- artis_ds %>%
  filter(
    (hs_version == "HS96" & year <= 2003) |
      ...
      (hs_version == "HS12" & year >= 2013 & year <= max_year)
...
# # point to the dataset folder
#artis_ds <- arrow::open_dataset(file.path(db_folder, "datasets", "ARTIS_consumption_v1.2.0_SAU_mid_2025-11-12.parquet"))
parquet_files <- list.files(
    file.path(db_folder, "outputs_combined"), 
    pattern = glue::glue("ARTIS_v{artis_version}_consumption_{prod_type}_mid_all_HS_yrs_*"),
    full.names = TRUE)
...
if(prod_type == "SAU"){
  prod <- fread(file.path(db_folder, "attribute_tables", "prod_sau.csv"), data.table = FALSE)
  # prod <- data.table::fread(
  # file.path("/Users/theamarks/Documents/UW-SAFS/ARTIS/data/", "model_inputs_1.2.0_SAU", "standardized_combined_prod.csv")) %>% 
  # rename(sciname = SciName,
  #       source_country_iso3c = country_iso3_alpha,
  #       method = prod_method,
  #       live_weight_t = quantity)
} else if (prod_type == "FAO"){
  prod <- fread(file.path(db_folder, "attribute_tables", "prod.csv"), data.table = FALSE)
}

baci <- fread(file.path(db_folder, "attribute_tables", "baci.csv"), data.table = FALSE) 
sciname <- fread(file.path(db_folder, "attribute_tables", "sciname_metadata.csv"), data.table = FALSE)
code_max_resolved_taxa <- fread(file.path(db_folder, "attribute_tables", "code_max_resolved_taxa.csv"), data.table = FALSE)

## FIXTIT
# Add some basic HS / year validation tests here. Do values fit into expected ranges?
```
...
baci <- artis_hs_summary %>%
  ...
  # ensure baci years align with artis
