#' Rebuild FAO Global Production Data from 2023 Format
#'
#' Processes FAO Global Fisheries Production data from the 2023 zip file format,
#' extracting and joining time series data with code list files to create a 
#' standardized production dataset compatible with ARTIS model workflows. Can be used for 
...
#'   \itemize{
#'     \item country - Country UN code
#'     \item isscaap_group - ISSCAAP group name in English
#'     \item quantity - Production quantity value
#'     \item year - Production year
#'     \item prod_method - Production source detail code
#'     \item habitat - Inland/marine group classification
#'   }
#'
#' @details 
#' This function performs the following operations:
#' \itemize{
#'   \item Unzips the FAO data archive if not already extracted
#'   \item Reads the Data Structure Definition (DSD) file to understand data organization
#'   \item Processes concept IDs and code IDs for proper column mapping
#'   \item Iteratively joins time series data with corresponding code list files
#'   \item Standardizes column names to match ARTIS 2020 data structure
#'   \item Converts "MEASURE" to "UNIT_CODE" and "STAUS" to "SYMBOL" for compatibility
#' }
#' 
#' The function handles factor/character conversions during joins and renames
#' columns to maintain consistency with existing ARTIS model code developed for 2020 data.
#'
#' @note This function expects the FAO zip file to contain:
#' \itemize{
#'   \item A DSD.xlsx file describing the data structure
#'   \item A time series CSV file containing production quantities
#'   \item Multiple code list (CL) files for joining classification data
#' }
#'
#' @examples
#' \dontrun{
#' # Process FAO 2023 production data
#' fao_data <- rebuild_fao_2023_dat(
#'   datadir = "path/to/fao/data",
#'   filename = "GlobalProduction_2023_1.0.0.zip"
#' )
#' }
#'
...
rebuild_fao_2023_dat <- function(datadir, filename) {
  ...
  if (dir.exists(unzip_folder) == FALSE) {
    unzip(
      zipfile = file.path(datadir, filename),
      exdir = unzip_folder,
      overwrite = TRUE
    )
  }
...
# Make code ids explicit --------------------------------------------------
  ...
  code_ids_to_change <- ds$Codelist_Code_id[
    ds$Codelist_Code_id == "CODE" & is.na(ds$Codelist_Code_id) == FALSE
  ]
  concept_ids_to_append <- ds$Concept_id_simple[
    ds$Codelist_Code_id == "CODE" & is.na(ds$Codelist_Code_id) == FALSE
  ]
  ...
  time_series_join <- time_series %>%
    rename("UNIT_CODE" = "MEASURE", "SYMBOL" = "STATUS") %>%
  ...
  for (i in 1:nrow(ds)) {
    ...
    if (!is.na(ds$Codelist_id[i])) {
      ...
      code_file_i <- fish_files[str_detect(
        string = fish_files,
        pattern = ds$Codelist_id[i]
      )]
      cl_i <- read.csv(
        file.path(unzip_folder, code_file_i),
        check.names = FALSE
      ) # check.names = FALSE to prevent R from adding "X" in front of column names that begin with numeric (e.g., 2020 version had column "3Alpha_Code")
      ...
      concept_names <- paste(
        ds$Concept_id_simple[i],
        names(cl_i)[
          grepl("name|major_group|identifier|code|description", names(cl_i)) &
      ...
      if (is.factor(cl_i[[merge_col]])) {
        ...
        time_series_join[[names(time_series_join)[
          i
