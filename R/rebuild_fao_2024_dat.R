...
# Get files ---------------------------------------------------------------
  ...
  unzip_folder_path <- file.path(datadir, str_remove(filename, ".zip"))
  # Test if file was already unzipped
  if (dir.exists(unzip_folder_path)==FALSE){
    unzip(zipfile = file.path(datadir, filename), 
          exdir = unzip_folder_path, 
          overwrite = TRUE)
  }
  
  fish_files <- list.files(unzip_folder_path)

# Clean DSD file ----------------------------------------------------------
  
  # the DSD.xlsx file explains the data structure of time series
  # each row gives info for how this time series column should be merged 
  # with a code list (CL) file
  ds_file <- fish_files[grep("DSD", fish_files)]
  
  # skip removes title row
  ds <- read_excel(file.path(unzip_folder_path, ds_file), skip=1)
  
  # Manually correct different parts of the data structure file:
  ds <- ds %>%
    # Manually correct data structure file: 
    # filename CL_FI_UNIT is actually FSJ_UNIT
    mutate(Codelist_id = if_else(Codelist_id == "CL_FI_UNIT", 
                                 true = "FSJ_UNIT", 
                                 false = Codelist_id)) %>%
    # Change "Measure" to "Unit_Code" and "Staus" to "Symbol" to match 
    # 2020 version - i.e. so that rest of ARTIS code (developed in 2020) works
    mutate(Concept_id = if_else(Concept_id == "MEASURE", 
                                true = "UNIT_CODE", 
                                false = Concept_id)) %>%
    mutate(Concept_id = if_else(Concept_id == "STAUS", 
                                true = "SYMBOL", 
                                false = Concept_id)) %>%
    # Create a simplifed version of concept_id names - 
    # remove everything after "." and "_"
    mutate(Concept_id_simple = Concept_id) %>%
    mutate(Concept_id_simple = str_replace(Concept_id_simple, 
                                           pattern = "\\..*", 
                                           replacement = "")) %>%
    mutate(Concept_id_simple = str_replace(Concept_id_simple, 
                                           pattern = "\\_.*", 
                                           replacement = "")) %>%
    # Convert all relevant columns to lowercase to avoid errors 
    # in matching upper/lowercase
    mutate(Concept_id_simple = tolower(Concept_id_simple))

# Make code ids explicit --------------------------------------------------
  ...
  code_ids_to_change <- ds$Codelist_Code_id[ds$Codelist_Code_id == "CODE" &
                                              is.na(ds$Codelist_Code_id) == FALSE]
  concept_ids_to_append <- ds$Concept_id_simple[ds$Codelist_Code_id == "CODE" &
                                                  is.na(ds$Codelist_Code_id) == FALSE]
  new_code_ids <- paste(concept_ids_to_append, code_ids_to_change, sep = "_")
  ds$Codelist_Code_id[ds$Codelist_Code_id == "CODE" &
                        is.na(ds$Codelist_Code_id) == FALSE] <- new_code_ids
  ds$Codelist_Code_id <- tolower(ds$Codelist_Code_id)
  

# Prep and Join timeseries and CL files ------------------------------------------------
  
  # read in timeseries
  # Was labeled as "TS" for time series in 2020 version
  time_file <- fish_files[grep("quantity", fish_files, ignore.case = TRUE)] 
  time_series <- fread(file.path(unzip_folder_path, time_file))
  
  # IMPORTANT: row ORDER (ABCDEF) in DSD file should match columns ABCDEF 
  # in time series for looping to work below
  ...
  # loop through CL files indicated in ds file - prep and join
  for (i in 1:nrow(ds)) {
    ...
    if (!is.na(ds$Codelist_id[i])) {
      ...
      code_file_i <- fish_files[str_detect(string = fish_files, 
                                           pattern = ds$Codelist_id[i])]
      # check.names = FALSE to prevent R from adding "X" in front of column names 
      # that begin with numeric (e.g., 2020 version had column "3Alpha_Code")
      cl_i <- fread(file.path(unzip_folder_path, code_file_i), 
                    check.names = FALSE) 
      ...
      # To disambiguate, append "Concept_id_simple" from DS file to all columns 
      # in CL that contain these terms
      # Exception to this is if name appears in ds$Codelist_code_id 
      # (i.e., keep UN_Code as is... don't change this to country_code_un)
      concept_names <- paste(ds$Concept_id_simple[i], 
      ...
      if (is.factor(cl_i[[merge_col]])){
        ...
        time_series_join[[names(time_series_join)[i]]] <- 
          as.character(time_series_join[[names(time_series_join)[i]]])
      }
      
      # Merge by column name
      # Note: the following code does not work: 
      # time_series_join<-left_join(time_series, cl_i, by = c(names(time_series)[i] = merge_col))
      # To make the argument "by" dynamic for each loop, 
      # it should be formatted as a named character as shown below
      ...
      if (is.character(time_series_join[[names(time_series_join)[i]]])){
        time_series_join[[names(time_series_join)[i]]] <- 
          as.factor(time_series_join[[names(time_series_join)[i]]])
      }
    }
