...
rebuild_fao_2024_dat <- function(datadir, filename){
  ...
  if (dir.exists(unzip_folder_path)==FALSE){
    unzip(zipfile = file.path(datadir, filename), exdir = unzip_folder_path, overwrite = TRUE)
  }
  ...
  time_series <- read.csv(file.path(unzip_folder_path, time_file))
  ...
  for (i in 1:nrow(ds)) {
    ...
    if (!is.na(ds$Codelist_id[i])) {
      ...
      cl_i <- read.csv(file.path(unzip_folder_path, code_file_i), check.names = FALSE) # check.names = FALSE to prevent R from adding "X" in front of column names that begin with numeric (e.g., 2020 version had column "3Alpha_Code")
