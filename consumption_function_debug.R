...
calculate_consumption <- function(artis, prod, curr_year, curr_hs_version,
  ...
  # FIXIT: Remove if decided we don't need to account for processing losses
  # V2_long <- V2
  # rownames(V2_long) <- colnames(V2_long)
  # V2_long <- as.data.frame(V2_long) %>%
  #   rownames_to_column(var = "hs6_processed") %>%
  #   pivot_longer(cols = -hs6_processed,
  #                names_to = "hs6_original",
  #                values_to = "processing_cf") %>%
  #   mutate(hs6_original = as.numeric(hs6_original),
  #          hs6_processed = as.numeric(hs6_processed))
  
  # FIXIT: This calculation can be simplified by using reweight_W
  processed_imports <- imports %>%
    ...
    # FIXIT: unsure if we need to account for processing losses here
    # left_join(V2_long,
    #           by = c("hs6" = "hs6_original", "hs6_processed")) %>%
    # mutate(processed_product_t = import_product_t * estimated_W * processing_cf) %>%
    ...
    mutate(total_hs6_processed_product = sum(processed_product_t)) %>%
    ungroup() %>%
    mutate(prop_processed_to_original_product = processed_product_t / total_hs6_processed_product)
  ...
  data_check_processed <- processed_imports %>%
    group_by(importer_iso3c, hs6_processed) %>%
    summarize(prop = sum(prop_processed_to_original_product)) %>%
    ungroup() %>%
  ...
  w_i <- processed_imports %>%
    ...
    group_by(importer_iso3c, hs6_processed) %>%
    summarize(processed_product_t = sum(processed_product_t, na.rm = TRUE)) %>%
    ungroup()
  
  # Calculate each country's consumption of foreign products as final hs6 
  consumption_foreign <- w_i %>%
    ...
    mutate(foreign_consumption_product_t = processed_product_t - foreign_export_product_t)
